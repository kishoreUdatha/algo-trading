...
import com.example.algo.common.model.enums.OrderStatus;
...
public class MultiTenantExecutionService {
  ...
  public Mono<Optional<Order>> placeOrderForUser(StrategySignalEvent signal, String userId) {
    ...
    if (!currentUser.getUserId().equals(userId)
        && !currentUser.hasPermission("PLACE_ORDER_FOR_OTHERS")) {
      throw new SecurityException(
          "User " + currentUser.getUserId() + " cannot place orders for user " + userId);
    }
  ...
  private Order buildUserOrder(StrategySignalEvent signal, String tradeId, String userId) {
    return Order.builder()
        ...
        .side(signal.getSide())
        .quantity(calculateQuantityForUser(signal, userId))
  ...
  private int calculateQuantityForUser(StrategySignalEvent signal, String userId) {
    ...
    UserPosition existingPosition =
        positionRepository.findBySymbolForCurrentUser(signal.getSymbol()).orElse(null);
  ...
  private void updateUserPosition(Order order, String userId) {
    try {
      ...
      log.info(
          "Updated position for user {} symbol {}: quantity={}",
          userId,
          order.getSymbol(),
          newQuantity);
  ...
  private void logOrderAttempt(String tradeId, StrategySignalEvent signal, String userId) {
    UserOrderAudit audit =
        UserOrderAudit.builder()
            .userId(userId)
            .tenantId(UserContext.getCurrentUser().getTenantId())
            .tradeId(tradeId)
  ...
  private void logOrderSuccess(Order order, String userId) {
    UserOrderAudit audit =
        UserOrderAudit.builder()
            ...
            .status(order.getStatus()) // This is already a String from deprecated method
            .createdAt(Instant.now())
            .build();
  ...
  private record OrderExecutionContext(
      String brokerName, Order order, StrategySignalEvent signal, String userId, String tradeId) {}
}
