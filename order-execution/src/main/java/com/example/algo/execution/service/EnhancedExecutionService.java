    ...
    return Mono.fromCallable(
        ...
        .doOnSuccess(
            result -> {
              if (result.isPresent()) {
                ...
                riskRules.updatePosition(
                    order.getSymbol(), order.getSide(), order.getPrice() * order.getQty());

                // Log successful order
                auditService.logOrderSuccess(tradeId, order);
                log.info(
                    "Order placed successfully: {} for signal from {}",
                    order.getId(),
                    signal.getStrategy());
              } else {
        ...
        .doOnError(
            error -> {
              ...
              log.error(
                  "Order placement failed for signal from {}: {}",
                  signal.getStrategy(),
                  error.getMessage());
            })
